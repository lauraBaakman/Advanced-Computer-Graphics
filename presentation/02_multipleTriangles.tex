...
\begin{frame}\frametitle{Properties}
	...
	\end{quote}
	\note<1>{\textbf{[Rick]} Problem when combining multiple triangles, so this is a important property}
\end{frame}
...
\begin{frame}\frametitle{Continuity}	
	...
	\end{itemize}
	\note<1>{\textbf{[Rick]} Continuity C0 is important -> no gaps. Higher is better because this gives a more smooth result.}
\end{frame}
...
\begin{frame}\frametitle{Sharp Edges}
	\begin{columns}
		...
		\end{column}
	\end{columns}
	\note<1>{\textbf{[Rick]} Curved triangles do not always give the preferred results -> sharp edges. Solution is to insert more triangles at the sharp edges -> model needs to be changed :(}
\end{frame}	
...
\begin{frame}\frametitle{Separate Normals}
	\begin{columns}
		...
		\end{column}
	\end{columns}
	\note<1>{\textbf{[Rick]} Beyond the scope of the paper extension exist to overcome the problem what you have when combining multiple meshes. Story about shared vertices.}
\end{frame}
