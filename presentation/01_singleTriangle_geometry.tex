...
\begin{frame}\frametitle{Overview}
	...
	\note{\textbf{[Laura]} PN triangle is defined by geometry and normal component.}
\end{frame}	
...
\begin{frame}\frametitle{Geometry - Tangent Coefficients}
	...
	\end{columns}
	\note<1>{\textbf{[Laura]} How to get the tangent coefficient (the ones on the edge but now curvy)}
	\note<2>{\textbf{[Laura]} Projection of the initial control points on the normal plane of a vertex.}
\end{frame}
...
\begin{frame}\frametitle{Geometry - Center Coefficient}
	...
	\note<2>{\textbf{[Laura]} Average of the tangent coefficients plus  half the difference between the tangent and vertex coefficients. -> why?}
\end{frame}	
...
\begin{frame}\frametitle{Cubic B\'ezier patch}
	\begin{columns}
		...
		\end{column}		
	\end{columns}
	\note<1>{\textbf{[Rick]} Very nice formula with a nice picture. Menstion \textbf{Barycentric coordinates}}
\end{frame}