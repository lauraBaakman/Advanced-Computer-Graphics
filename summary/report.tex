\documentclass[a4paper, twocolumn, fleqn]{article}
\usepackage[english]{babel}
\usepackage{a4wide}

% Fonts
\usepackage[sc]{mathpazo}
\usepackage[T1]{fontenc}
\linespread{1.05}
\usepackage{lmodern}
\usepackage{microtype}
\usepackage{lettrine}

% Floats
\usepackage{float}
\usepackage[section]{placeins}

%Plaatjes
\usepackage{tikz}
\usepackage{graphicx}
\usepackage[small,labelfont=bf,up,textfont=sl,up]{caption}
\usepackage{subcaption}
\DeclareCaptionLabelFormat{opening}{(#2)}
\captionsetup{subrefformat=opening}


% Custom section headers
\usepackage{titlesec}
\titleformat{\section}[block]{\large\scshape}{\thesection.}{1em}{}
\titleformat{\subsubsection}[block]{\scshape}{\thesubsubsection.}{1em}{}

% References
\usepackage{varioref}
\usepackage{hyperref}
\usepackage[noabbrev]{cleveref}

% Plaatje boven titel
\usepackage{titling}

% References
\usepackage[backend=bibtex]{biblatex}

\usepackage{csquotes}
\bibliography{biblio}

% Temp
\usepackage[obeyFinal]{todonotes}

\predate{\centering\large}
\postdate{%
	\centering
	\vspace{2ex}
  	~\\\includegraphics[width=0.9\textwidth]{img/plaatje.png}
  	\captionof{figure}{From left to right input triangulation, the Gouraud shaded input triangulation, the geometric component of the PN triangles, shaded according to surface normal variation, and the curved PN triangles, shaded with quadratically varying normals. Image is adapted from \textcite{vlachos2001curved}.}
  	\label{fig:sum:result}
}

\title{Curved PN Triangles}
\author{%
	Rick van Veen (s1883933)%
	%\thanks{These authors contributed equally to this work.}% 
	\and% 
	Laura Baakman (s1869140)%
	%\footnotemark[1]%
}

\begin{document}

\maketitle

\input{body.tex}

\printbibliography

\end{document}